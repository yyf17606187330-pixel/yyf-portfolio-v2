...
const longFilms: LongFilmRecord[] = [
  {
    project: {
      slug: 'travel-vlog',
      title: '旅拍 Vlog',
...
export function LongFormProjects({ playerOpen, onOpenProject }: LongFormProjectsProps) {
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      if (nextIndex < 0 || nextIndex >= longFilms.length) return;
  ...
  const handleKeyDown = (event: ReactKeyboardEvent<HTMLDivElement>) => {
    ...
    if (['ArrowDown', 'ArrowRight', 'PageDown'].includes(event.key)) {
      ...
      nextIndex = longFilms.length - 1;
    }

    if (nextIndex !== null && transitionTo(nextIndex)) event.preventDefault();
  };

  const activeFilm = longFilms[activeIndex];
  const narrativeFilm = longFilms[1];
  const activeDescriptionId = 'long-form-active-description';
  const activeHasPreview = Boolean(activeFilm.project.previewSrc);
  const narrativePoster = resolveMediaUrl(narrativeFilm.project.poster);

  return (
    <section aria-label="长片作品" className="long-form-projects">
      <header className="long-form-projects__heading">
        <span>02.02—02.05</span>
      ...
      <div className="long-form-projects__intro">
        ...
        <p>
          ...
          四部作品呈现我在剪辑、调色、声音与现场执行之间的不同工作方式。
        </p>
      </div>
      ...
      >
        <article
          className="long-form-projects__feature long-form-projects__feature--deck"
          data-card-feature
        >
          <div className="long-form-projects__deck-column">
            <div
              aria-describedby="long-form-deck-help"
              aria-label="影像作品抽牌浏览"
              ...
              tabIndex={0}
            >
              <span className="long-form-projects__sr-only" id="long-form-deck-help">
                使用上下方向键、按钮或在卡片区域滚动切换作品。
              </span>
              {longFilms.map((film, index) => (
                <LongFilmCard
            ...
            <div className="long-form-projects__preview-bar">
              <span>
                {!activeHasPreview || reducedMotion ? '静态封面' : '8 秒静音预览'}
              </span>
            ...
            <div className="long-form-projects__controls">
              <p aria-live="polite">
                ...
                {' / '}{String(longFilms.length).padStart(2, '0')} · {activeFilm.project.title}
              </p>
              <div>
                <button
                  aria-label="上一张作品"
                ...
                <button
                  aria-label="下一张作品"
                  disabled={activeIndex === longFilms.length - 1}
                  onClick={() => transitionTo(activeIndexRef.current + 1)}
                  type="button"
                >
                  <span aria-hidden="true">↓</span>
        ...
        <article
          className="long-form-projects__feature long-form-projects__feature--narrative"
          data-static-feature="narrative-film"
        >
          <LongFilmCopy film={narrativeFilm} />
          <button
            aria-label={`播放下方${narrativeFilm.project.title}完整作品`}
            className="long-form-projects__static-media"
            onClick={(event) => onOpenProject(narrativeFilm.project, event.currentTarget)}
            type="button"
          >
            {narrativePoster ? (
              <img
            ...
            <span aria-hidden="true">播放正片</span>
          </button>
        </article>
      </div>
    </section>
