...
describe('App', () => {
  ...
  it('keeps only live navigation links after retiring the placeholder contact panel', () => {
    ...
    expect(links.map((link) => link.textContent)).toEqual(['WORK', 'ABOUT', 'EXPERIENCE']);
  ...
  it('retains the committed poster and loads original Hero motion when the observer is unavailable', () => {
    const { container } = render(<App />);
    expect(screen.getByRole('img', { name: '杨玉峰个人肖像' })).toHaveAttribute('src', '/media/hero/hero-poster.webp');
    expect(container.querySelector('.p5-hero video')).toHaveAttribute('src', '/media/p5/hero/hero-scroll.mp4');
    expect(container.querySelector('.p5-hero video')).toHaveAttribute('poster', '/media/hero/hero-poster.webp');
  });
  ...
  it('resolves Hero media through the configured deployment media base', () => {
    ...
    const { container } = render(<App />);
    expect(screen.getByRole('img', { name: '杨玉峰个人肖像' })).toHaveAttribute('src', 'https://cdn.example.com/portfolio/hero/hero-poster.webp');
    expect(container.querySelector('.p5-hero video')).toHaveAttribute('src', 'https://cdn.example.com/portfolio/p5/hero/hero-scroll.mp4');
  });

  it('opens a directly selected film and restores its play button after Escape', async () => {
    render(<App />);
    fireEvent.click(screen.getByRole('button', { name: '选择棋局短片' }));
    const opener = screen.getByRole('button', { name: '播放棋局短片完整作品' });
    fireEvent.click(opener);
    const dialog = screen.getByRole('dialog', { name: '播放作品：棋局短片' });
    expect(dialog.querySelector('video')).toHaveAttribute('src', '/media/projects/long-form/film-2025-06-15/full-hevc.mp4');
    fireEvent.keyDown(document, { key: 'Escape' });
