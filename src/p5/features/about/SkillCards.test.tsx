...
describe('SkillCards', () => {
  it('keeps all six categories visible and exposes every approved skill and evidence link on selection', () => {
    ...
    for (const group of aboutContent.capabilityGroups) {
      ...
      const panel = screen.getByRole('region', { name: group.title });
      for (const detail of group.details ?? []) {
        expect(within(panel).getByText(detail.title)).toBeInTheDocument();
        expect(within(panel).getByText(detail.body)).toBeInTheDocument();
      }
      expect(within(panel).getByText(group.practice!.title)).toBeInTheDocument();
      expect(within(panel).getByText(group.practice!.body)).toBeInTheDocument();
      for (const inactive of aboutContent.capabilityGroups.filter((item) => item.id !== group.id)) {
        expect(within(panel).queryByText(inactive.practice!.title)).not.toBeInTheDocument();
      }
    }
